...
export default function ProximosPartidosArg() {
  const [partidos, setPartidos] = useState<Partido[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(
      "https://www.thesportsdb.com/api/v1/json/123/eventsnextleague.php?id=4406"
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <Paper sx={{ p: 2, mb: 2 }}>
      <Typography variant="h6" fontWeight="bold" sx={{ mb: 2 }}>
        Próximos Partidos - Primera División Argentina
      </Typography>
